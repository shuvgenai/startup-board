...
| Market research brief (medium/heavy) | On request or once idea is past fresh-idea stage | Markdown file artifact (or `.docx` via the docx skill if the founder wants a polished, shareable document) — this is meant to be saved and referenced, not read once in chat |
| Positioning/competitor snapshot | When 2–3+ competitors were researched | `comparison_card_display_v0` tool if attributes line up cleanly, otherwise a short markdown table |
